...
def is_tw_market_open():
    """檢查台股是否開盤（讀取 market_status.json）"""
    status_file = "/home/jhe/.openclaw/workspace/stock/market_status.json"
    try:
        with open(status_file, 'r') as f:
            data = json.load(f)
        tw = data.get('tw', {})
        # holiday=true 或 status="休市中" → 不交易
        if tw.get('holiday') or tw.get('status') == '休市中':
            return False
        # 檢查時間（台股 09:00-13:30）
        now = datetime.now()
        h, m = now.hour, now.minute
        time_min = h * 60 + m
        if time_min < 9 * 60 or time_min > 13 * 60 + 30:
            return False
        return True
    except Exception:
        # 如果讀不到，用時間判斷（fallback）
        now = datetime.now()
        if now.weekday() >= 5:  # 週末
            return False
        h, m = now.hour, now.minute
        time_min = h * 60 + m
        return 9 * 60 <= time_min <= 13 * 60 + 30
...
def fetch_page_data():
    ...
    name_pat = re.compile(
        r'Ell">([^<]{2,20})</div><div class="D\(f\) Ai\(c\)"><span class="Fz\(14px\) C\(#979ba7\) Ell">([A-Z0-9]{4,6})\.TW</span>')
...
def main():
    ...
    if not is_tw_market_open():
        now = datetime.now()
        print(f"[{now.strftime('%H:%M')}] 台股已收盤（13:30 以後），跳過執行")
        return
