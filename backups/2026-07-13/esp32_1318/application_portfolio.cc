...
PortfolioApplication::PortfolioApplication()
    ...
    , lbl_portfolio_cost_(nullptr)
    , lbl_portfolio_today_(nullptr)
    , lbl_portfolio_tw_(nullptr)
    , lbl_portfolio_us_(nullptr)
    , lbl_portfolio_div_(nullptr)
    , lbl_portfolio_yield_(nullptr)
    , lbl_portfolio_updated_(nullptr)
    , lbl_cash_(nullptr)
    , lbl_fx_(nullptr)
    , lbl_fx_usd_(nullptr)
    , lbl_fx_usd_twd_(nullptr)
    , lbl_fx_jpy_twd_(nullptr)
    , lbl_solar_(nullptr)
...
void PortfolioApplication::UpdateHomeCards() {
    ...
    if (lbl_weather_updated_) {
        ...
        ESP_LOGI(TAG, "[DEBUG] UpdateHomeCards: weather_.updated=[%s] (len=%d)", up, (int)strlen(up));
    ...
    // Also update detail weather screen updated label
    if (det_weather_updated_) {
        const char *up = weather_.updated;
        if (up[0] == '"') up++;
        char *p = strstr(up, "T");
        if (!p) p = strstr(up, " ");
        char buf2[64];
        if (p) {
            snprintf(buf2, sizeof(buf2), "更新: %.5s", p + 1);
        } else {
            snprintf(buf2, sizeof(buf2), "更新: %s", up);
        }
        lv_label_set_text(det_weather_updated_, buf2);
    }
    ...
    if (lbl_portfolio_cost_) {
        char buf[64], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), summary_.total_cost);
        snprintf(buf, sizeof(buf), "成本 $%s", tmp);
        lv_label_set_text(lbl_portfolio_cost_, buf);
    }
    if (lbl_portfolio_today_) {
        char buf[64], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), fabs(summary_.today_change));
        snprintf(buf, sizeof(buf), "今日 %s%s (%+.2f%%)",
            summary_.today_change >= 0 ? "+" : "-", tmp, summary_.today_change_pct);
        lv_label_set_text(lbl_portfolio_today_, buf);
        lv_obj_set_style_text_color(lbl_portfolio_today_,
            summary_.today_change >= 0 ? lv_color_hex(COLOR_POSITIVE) : lv_color_hex(COLOR_NEGATIVE), 0);
    }
    if (lbl_portfolio_tw_) {
        char buf[64], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), tw_.mktval);
        snprintf(buf, sizeof(buf), "台股 $%s (%+.2f%%)", tmp, tw_.gain_pct);
        lv_label_set_text(lbl_portfolio_tw_, buf);
        lv_obj_set_style_text_color(lbl_portfolio_tw_,
            tw_.gain >= 0 ? lv_color_hex(0xFFD54F) : lv_color_hex(COLOR_NEGATIVE), 0);
    }
    if (lbl_portfolio_us_) {
        char buf[64], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), us_.mktval);
        snprintf(buf, sizeof(buf), "美股 $%s (%+.2f%%)", tmp, us_.gain_pct);
        lv_label_set_text(lbl_portfolio_us_, buf);
        lv_obj_set_style_text_color(lbl_portfolio_us_,
            us_.gain >= 0 ? lv_color_hex(0xCE93D8) : lv_color_hex(COLOR_NEGATIVE), 0);
    }
    if (lbl_portfolio_div_) {
        char buf[64], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), summary_.annual_div);
        snprintf(buf, sizeof(buf), "配息 $%s/年 %s", tmp,
            summary_.yield_cost[0] ? summary_.yield_cost : "--");
        lv_label_set_text(lbl_portfolio_div_, buf);
    }
    if (lbl_portfolio_yield_) {
        char buf[64];
        snprintf(buf, sizeof(buf), "成本 %s  |  現價 %s",
            summary_.yield_cost[0] ? summary_.yield_cost : "--",
            summary_.yield_cur[0] ? summary_.yield_cur : "--");
        lv_label_set_text(lbl_portfolio_yield_, buf);
    }
    if (lbl_portfolio_updated_) {
        char buf[64];
        snprintf(buf, sizeof(buf), "資料: %s", fx_.updated[0] ? fx_.updated : "--");
        lv_label_set_text(lbl_portfolio_updated_, buf);
    }
    ...
    // 持倉漲跌排行 labels
    for (int i = 0; i < 4; i++) {
        if (!lbl_mover_[i]) continue;
        if (summary_.movers_[i].symbol[0]) {
            char buf[64];
            snprintf(buf, sizeof(buf), "%d. %s %+.2f%%", i+1,
                summary_.movers_[i].symbol, summary_.movers_[i].chg_pct);
            lv_label_set_text(lbl_mover_[i], buf);
            lv_obj_set_style_text_color(lbl_mover_[i],
                summary_.movers_[i].chg_pct >= 0 ? lv_color_hex(COLOR_POSITIVE) : lv_color_hex(COLOR_NEGATIVE), 0);
        } else {
            lv_label_set_text(lbl_mover_[i], "--");
            lv_obj_set_style_text_color(lbl_mover_[i], lv_color_hex(COLOR_SUBTEXT), 0);
        }
    }
    if (lbl_mover_summary_) {
        char buf[48];
        snprintf(buf, sizeof(buf), "漲 %d  /  跌 %d", summary_.up_count, summary_.dn_count);
        lv_label_set_text(lbl_mover_summary_, buf);
    }
    ...
    if (lbl_fx_usd_twd_) {
        // Show "USD 現金 NT$ XXX" using cash_.usd_in_twd (already computed by gen script)
        char buf[48], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), cash_.usd_in_twd);
        snprintf(buf, sizeof(buf), "USD 現金 NT$ %s", tmp);
        lv_label_set_text(lbl_fx_usd_twd_, buf);
    }

    if (lbl_fx_jpy_twd_) {
        char buf[48], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), cash_.jpy_in_twd);
        snprintf(buf, sizeof(buf), "JPY 現金 NT$ %s", tmp);
        lv_label_set_text(lbl_fx_jpy_twd_, buf);
    }
    ...
    if (det_tw_val_) {
        char buf[64], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), tw_.mktval);
        snprintf(buf, sizeof(buf), "$%s  %s%.0f (%+.2f%%)",
            tmp,
            tw_.gain >= 0 ? "+" : "-", fabs(tw_.gain), tw_.gain_pct);
    ...
    if (det_us_val_) {
        ...
        snprintf(buf, sizeof(buf), "$%s  %s%.0f (%+.2f%%)",
            tmp,
            us_.gain >= 0 ? "+" : "-", fabs(us_.gain), us_.gain_pct);
    ...
    if (det_today_change_) {
        char buf[64], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), fabs(summary_.today_change));
        snprintf(buf, sizeof(buf), "今日 %s%s (%+.2f%%)",
            summary_.today_change >= 0 ? "+" : "-", tmp, summary_.today_change_pct);
        lv_label_set_text(det_today_change_, buf);
        lv_obj_set_style_text_color(det_today_change_,
            summary_.today_change >= 0 ? lv_color_hex(COLOR_POSITIVE) : lv_color_hex(COLOR_NEGATIVE), 0);
    }
    if (det_annual_div_) {
        char buf[64], tmp[32];
        FormatWithCommas(tmp, sizeof(tmp), summary_.annual_div);
        snprintf(buf, sizeof(buf), "$%s / 年", tmp);
        lv_label_set_text(det_annual_div_, buf);
    }
    if (det_portfolio_updated_) {
        char buf[64];
        snprintf(buf, sizeof(buf), "資料: %s", fx_.updated[0] ? fx_.updated : "--");
        lv_label_set_text(det_portfolio_updated_, buf);
    }
    if (det_cash_usd_) {
        char buf[64];
...
void PortfolioApplication::ParsePortfolioJson(const char *json) {
    ...
    // Parse TW block (anchored at "tw" to avoid matching other fields)
    p = strstr(flat, "\"tw\"");
    if (p) {
    ...
    ESP_LOGI(TAG, "TW: mktval=%.0f gain=%.0f (%.2f%%)",
        tw_.mktval, tw_.gain, tw_.gain_pct);

    // Parse US block (anchored at "us" — note: "us" key comes after summary gain_pct)
    p = strstr(flat, "\"us\"");
    if (p) {
        const char *cp = strstr(p, "\"mktval_twd\":");
        if (cp) sscanf(cp, "\"mktval_twd\":%f", &us_.mktval);
        cp = strstr(p, "\"gain_twd\":");
        if (cp) sscanf(cp, "\"gain_twd\":%f", &us_.gain);
        cp = strstr(p, "\"gain_pct\":");
        if (cp) sscanf(cp, "\"gain_pct\":%f", &us_.gain_pct);
    }
    ESP_LOGI(TAG, "US: mktval=%.0f gain=%.0f (%.2f%%)",
        us_.mktval, us_.gain, us_.gain_pct);

    // Parse FX block (anchored at "fx" to avoid matching cash.rate_usd_twd)
    // Note: "usd_twd"/"jpy_twd" appear BOTH in fx and as `rate_usd_twd`/`rate_jpy_twd` in cash.
    // Without anchoring at "fx", strstr finds the cash block first.
    p = strstr(flat, "\"fx\"");
    if (p) {
        const char *cp = strstr(p, "\"usd_twd\":");
        if (cp) sscanf(cp, "\"usd_twd\":%f", &fx_.usd_twd);
        cp = strstr(p, "\"jpy_twd\":");
        if (cp) sscanf(cp, "\"jpy_twd\":%f", &fx_.jpy_twd);
        // fx.updated field (e.g. "2026-07-13 07:30:31")
        const char *up = strstr(p, "\"updated\": \"");
        if (up) {
            up += 12;  // skip past "updated": "
            const char *end = strchr(up, '"');
            if (end) {
                int len = (int)(end - up);
                if (len > (int)sizeof(fx_.updated) - 1) len = sizeof(fx_.updated) - 1;
                strncpy(fx_.updated, up, len);
                fx_.updated[len] = '\0';
            }
        }
    }
    ESP_LOGI(TAG, "FX: usd_twd=%.4f jpy_twd=%.4f updated=[%s]",
        fx_.usd_twd, fx_.jpy_twd, fx_.updated);

    // Parse top-level summary fields (today_change, annual_div, yield_cost/cur)
    p = strstr(flat, "\"today_change\":");
    if (p) sscanf(p, "\"today_change\":%f", &summary_.today_change);
    p = strstr(flat, "\"today_change_pct\":");
    if (p) sscanf(p, "\"today_change_pct\":%f", &summary_.today_change_pct);
    p = strstr(flat, "\"annual_div\":");
    if (p) sscanf(p, "\"annual_div\":%f", &summary_.annual_div);

    // String fields: yield_cost, yield_cur (e.g. "5.41%")
    // Match key, skip past colon and any whitespace, then read until closing quote
    const char *yc = strstr(flat, "\"yield_cost\":");
    if (yc) {
        yc += strlen("\"yield_cost\":");
        while (*yc == ' ' || *yc == '\t') yc++;
        if (*yc == '"') {
            yc++;
            const char *yc_end = strchr(yc, '"');
            if (yc_end) {
                int len = (int)(yc_end - yc);
                if (len > (int)sizeof(summary_.yield_cost) - 1) len = sizeof(summary_.yield_cost) - 1;
                strncpy(summary_.yield_cost, yc, len);
                summary_.yield_cost[len] = '\0';
            }
        }
    }
    const char *yr = strstr(flat, "\"yield_cur\":");
    if (yr) {
        yr += strlen("\"yield_cur\":");
        while (*yr == ' ' || *yr == '\t') yr++;
        if (*yr == '"') {
            yr++;
            const char *yr_end = strchr(yr, '"');
            if (yr_end) {
                int len = (int)(yr_end - yr);
                if (len > (int)sizeof(summary_.yield_cur) - 1) len = sizeof(summary_.yield_cur) - 1;
                strncpy(summary_.yield_cur, yr, len);
                summary_.yield_cur[len] = '\0';
            }
        }
    }
    ESP_LOGI(TAG, "Today: %+.0f (%+.2f%%)  AnnualDiv=%.0f Yield(cost/cur)=[%s/%s]",
        summary_.today_change, summary_.today_change_pct,
        summary_.annual_div, summary_.yield_cost, summary_.yield_cur);

    // Parse movers (top N 漲跌排行)
    p = strstr(flat, "\"up_count\":");
    if (p) sscanf(p, "\"up_count\":%d", &summary_.up_count);
    p = strstr(flat, "\"dn_count\":");
    if (p) sscanf(p, "\"dn_count\":%d", &summary_.dn_count);
    p = strstr(flat, "\"total\":");
    if (p) sscanf(p, "\"total\":%d", &summary_.total_holdings);

    // Parse movers.top array
    const char *mp = strstr(flat, "\"top\":");
    if (mp) {
        const char *arr_start = strchr(mp, '[');
        const char *arr_end = arr_start ? strchr(arr_start, ']') : nullptr;
        if (arr_start && arr_end) {
            int idx = 0;
            const char *obj = arr_start;
            while (idx < 4 && obj < arr_end) {
                obj = strchr(obj + 1, '{');
                ...
                if (obj_len < sizeof(obj_buf)) {
                    ...
                    obj_buf[obj_len] = '\0';
                    auto &m = summary_.movers_[idx];
                    // symbol
                    const char *sp = strstr(obj_buf, "\"symbol\": \"");
                    if (!sp) sp = strstr(obj_buf, "\"symbol\":\"");
                    if (sp) {
                        sp = strchr(sp, '"'); sp++; sp++;
                        if (*sp == '"') sp++;
                        ...
                        if (se) {
                            int l = (int)(se - sp);
                            if (l > (int)sizeof(m.symbol) - 1) l = sizeof(m.symbol) - 1;
                            strncpy(m.symbol, sp, l);
                            m.symbol[l] = '\0';
                        }
                    }
                    // chg_pct
                    sp = strstr(obj_buf, "\"chg_pct\":");
                    if (sp) sscanf(sp, "\"chg_pct\":%f", &m.chg_pct);
                    idx++;
                }
                obj = obj_end;
            }
            ESP_LOGI(TAG, "Movers: %d parsed 漲%d/跌%d/共%d", idx,
                summary_.up_count, summary_.dn_count, summary_.total_holdings);
            for (int i = 0; i < idx; i++) {
                ESP_LOGI(TAG, "  [%d] %s %+.2f%%", i+1,
                    summary_.movers_[i].symbol, summary_.movers_[i].chg_pct);
            }
        }
    }

    // Parse Cash block (anchored at "cash")
    p = strstr(flat, "\"cash\"");
    if (p) {
        ...
        if (cp) {
            ...
            // 新增：rate_usd_twd → cash_.usd_rate
            ap = strstr(cp, "\"rate_usd_twd\":");
            if (ap) sscanf(ap, "\"rate_usd_twd\":%f", &cash_.usd_rate);
        }
        cp = strstr(p, "\"jpy\"");
        if (cp) {
            const char *ap = strstr(cp, "\"amount\":");
            ...
            // 新增：rate_jpy_twd → cash_.jpy_rate
            ap = strstr(cp, "\"rate_jpy_twd\":");
            if (ap) sscanf(ap, "\"rate_jpy_twd\":%f", &cash_.jpy_rate);
        }
    }

    ESP_LOGI(TAG, "Cash parse: USD=%.0f in_twd=%.0f rate=%.3f, JPY=%.0f in_twd=%.0f rate=%.3f",
        cash_.usd_amount, cash_.usd_in_twd, cash_.usd_rate,
        cash_.jpy_amount, cash_.jpy_in_twd, cash_.jpy_rate);
    ESP_LOGI(TAG, "Summary: mktval=%.0f gain=%.0f (%.2f%%)",
        summary_.total_mktval, summary_.total_gain, summary_.total_gain_pct);

    // Parse solar_kwh (top-level field, comes after fx block)
        ...
        if (arr_start && arr_end && arr_end > arr_start) {
            ...
            while (day_idx < Weather::FORECAST_DAYS && obj < arr_end) {
                ...
                if (obj_len < sizeof(obj_buf)) {
                    ...
                    const char *sp = strstr(obj_buf, "\"day\": \"");  // colon + space
                    if (!sp) sp = strstr(obj_buf, "\"day\":\"");  // colon no space (fallback)
                    if (sp) {
                        const char *s = sp + (sp[6] == ' ' ? 8 : 7);  // skip past "day": or "day": 
                        const char *e = strchr(s, '\"');
                        if (e) {
                            int l = (int)(e-s);
                            if (l > (int)sizeof(fc.day)-1) l = sizeof(fc.day)-1;
                            strncpy(fc.day, s, l);
                            fc.day[l] = '\0';
                        }
                    }
                    ESP_LOGI(TAG, "[DEBUG] parse day[%d]='%s'", day_idx, fc.day);
...
void PortfolioApplication::CreateHomeScreen() {
    ...
    // --- 持股卡片（左，滿高度）---
    card = CreateCard(home_scr_, 155, 325);
    lv_obj_set_pos(card, 8, 155);
    lv_obj_set_size(card, 232, 325);
    ...
    // y=58: total gain (with sign + %)
    lbl = lv_label_create(card);
    lv_label_set_text(lbl, "$ -- (--%)");
    lv_obj_set_pos(lbl, 10, 58);
    lv_obj_set_style_text_color(lbl, lv_color_hex(COLOR_POSITIVE), 0);
    lv_obj_set_style_text_font(lbl, &font_puhui_16_4, 0);
    lbl_portfolio_gain_ = lbl;

    // y=86: divider 1
    lv_obj_t *pf_div1 = lv_obj_create(card);
    lv_obj_set_pos(pf_div1, 10, 86);
    lv_obj_set_size(pf_div1, 212, 1);
    lv_obj_set_style_bg_color(pf_div1, lv_color_hex(COLOR_CARD_LINE), 0);
    lv_obj_set_style_border_width(pf_div1, 0, 0);
    lv_obj_remove_flag(pf_div1, LV_OBJ_FLAG_SCROLLABLE);

    // y=96: cost
    lbl_portfolio_cost_ = lv_label_create(card);
    lv_label_set_text(lbl_portfolio_cost_, "成本 $--");
    lv_obj_set_pos(lbl_portfolio_cost_, 10, 96);
    lv_obj_set_style_text_color(lbl_portfolio_cost_, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl_portfolio_cost_, &font_puhui_16_4, 0);

    // y=124: 今日漲跌 ← NEW
    lbl_portfolio_today_ = lv_label_create(card);
    lv_label_set_text(lbl_portfolio_today_, "今日 --");
    lv_obj_set_pos(lbl_portfolio_today_, 10, 124);
    lv_obj_set_style_text_color(lbl_portfolio_today_, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl_portfolio_today_, &font_puhui_16_4, 0);

    // y=152: 台股 split
    lbl_portfolio_tw_ = lv_label_create(card);
    lv_label_set_text(lbl_portfolio_tw_, "台股 $-- (--%)");
    lv_obj_set_pos(lbl_portfolio_tw_, 10, 152);
    lv_obj_set_style_text_color(lbl_portfolio_tw_, lv_color_hex(0xFFD54F), 0);
    lv_obj_set_style_text_font(lbl_portfolio_tw_, &font_puhui_16_4, 0);

    // y=180: 美股 split
    lbl_portfolio_us_ = lv_label_create(card);
    lv_label_set_text(lbl_portfolio_us_, "美股 $-- (--%)");
    lv_obj_set_pos(lbl_portfolio_us_, 10, 180);
    lv_obj_set_style_text_color(lbl_portfolio_us_, lv_color_hex(0xCE93D8), 0);
    lv_obj_set_style_text_font(lbl_portfolio_us_, &font_puhui_16_4, 0);

    // y=208: divider 2
    lv_obj_t *pf_div2 = lv_obj_create(card);
    lv_obj_set_pos(pf_div2, 10, 208);
    lv_obj_set_size(pf_div2, 212, 1);
    lv_obj_set_style_bg_color(pf_div2, lv_color_hex(COLOR_CARD_LINE), 0);
    lv_obj_set_style_border_width(pf_div2, 0, 0);
    lv_obj_remove_flag(pf_div2, LV_OBJ_FLAG_SCROLLABLE);

    // y=218: 年配息
    lbl_portfolio_div_ = lv_label_create(card);
    lv_label_set_text(lbl_portfolio_div_, "配息 $--/年 (--%)");
    lv_obj_set_pos(lbl_portfolio_div_, 10, 218);
    lv_obj_set_style_text_color(lbl_portfolio_div_, lv_color_hex(COLOR_MONEY), 0);
    lv_obj_set_style_text_font(lbl_portfolio_div_, &font_puhui_16_4, 0);

    // y=246: 成本 / 現價 脤利率
    lbl_portfolio_yield_ = lv_label_create(card);
    lv_label_set_text(lbl_portfolio_yield_, "成本 -- | 現價 --");
    lv_obj_set_pos(lbl_portfolio_yield_, 10, 246);
    lv_obj_set_style_text_color(lbl_portfolio_yield_, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl_portfolio_yield_, &font_puhui_16_4, 0);

    // y=274: divider 3
    lv_obj_t *pf_div3 = lv_obj_create(card);
    lv_obj_set_pos(pf_div3, 10, 274);
    lv_obj_set_size(pf_div3, 212, 1);
    lv_obj_set_style_bg_color(pf_div3, lv_color_hex(COLOR_CARD_LINE), 0);
    lv_obj_set_style_border_width(pf_div3, 0, 0);
    lv_obj_remove_flag(pf_div3, LV_OBJ_FLAG_SCROLLABLE);

    // y=284: 資料時間
    lbl_portfolio_updated_ = lv_label_create(card);
    lv_label_set_text(lbl_portfolio_updated_, "資料: --");
    lv_obj_set_pos(lbl_portfolio_updated_, 10, 284);
    lv_obj_set_style_text_color(lbl_portfolio_updated_, lv_color_hex(0x9E9E9E), 0);
    lv_obj_set_style_text_font(lbl_portfolio_updated_, &font_puhui_16_4, 0);
    ...
    // --- 現金卡片（右上）→ 改為「持倉漲跌排行」---
    card = CreateCard(home_scr_, 155, 158);
    lv_obj_set_pos(card, 248, 155);
    lv_obj_set_size(card, 224, 158);

    // y=5: title
    lbl = lv_label_create(card);
    lv_label_set_text(lbl, "持倉漲跌");
    lv_obj_set_pos(lbl, 10, 5);
    lv_obj_set_style_text_color(lbl, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl, &font_puhui_16_4, 0);

    // y=25, 48, 71, 94: 4 個 mover labels
    for (int i = 0; i < 4; i++) {
        lbl_mover_[i] = lv_label_create(card);
        lv_label_set_text(lbl_mover_[i], "--  --");
        lv_obj_set_pos(lbl_mover_[i], 10, 25 + i * 23);
        lv_obj_set_style_text_color(lbl_mover_[i], lv_color_hex(COLOR_TEXT), 0);
        lv_obj_set_style_text_font(lbl_mover_[i], &font_puhui_16_4, 0);
    }

    // y=120: divider
    lv_obj_t *mover_div = lv_obj_create(card);
    lv_obj_set_pos(mover_div, 10, 120);
    lv_obj_set_size(mover_div, 204, 1);
    lv_obj_set_style_bg_color(mover_div, lv_color_hex(COLOR_CARD_LINE), 0);
    lv_obj_set_style_border_width(mover_div, 0, 0);
    lv_obj_remove_flag(mover_div, LV_OBJ_FLAG_SCROLLABLE);

    // y=130: 漲跌統計
    lbl_mover_summary_ = lv_label_create(card);
    lv_label_set_text(lbl_mover_summary_, "漲-- / 跌--");
    lv_obj_set_pos(lbl_mover_summary_, 10, 130);
    lv_obj_set_style_text_color(lbl_mover_summary_, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl_mover_summary_, &font_puhui_16_4, 0);
    ...
    // --- 匯率卡片（右下）---
    card = CreateCard(home_scr_, 321, 159);
    lv_obj_set_pos(card, 248, 321);
    lv_obj_set_size(card, 224, 159);
    ...
    lv_obj_set_pos(lbl, 10, 52);
    lv_obj_set_style_text_color(lbl, lv_color_hex(0xCE93D8), 0);
    lv_obj_set_style_text_font(lbl, &font_puhui_16_4, 0);
    lbl_fx_ = lbl;

    // y=80: divider
    lv_obj_t *fx_div = lv_obj_create(card);
    lv_obj_set_pos(fx_div, 10, 80);
    lv_obj_set_size(fx_div, 204, 1);
    lv_obj_set_style_bg_color(fx_div, lv_color_hex(COLOR_CARD_LINE), 0);
    lv_obj_set_style_border_width(fx_div, 0, 0);
    lv_obj_remove_flag(fx_div, LV_OBJ_FLAG_SCROLLABLE);

    // y=88: USD×TWD 總值  ← NEW
    lbl_fx_usd_twd_ = lv_label_create(card);
    lv_label_set_text(lbl_fx_usd_twd_, "USD 現金 NT$ --");
    lv_obj_set_pos(lbl_fx_usd_twd_, 10, 88);
    lv_obj_set_style_text_color(lbl_fx_usd_twd_, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl_fx_usd_twd_, &font_puhui_16_4, 0);

    // y=115: JPY×TWD 總值 ← NEW
    lbl_fx_jpy_twd_ = lv_label_create(card);
    lv_label_set_text(lbl_fx_jpy_twd_, "JPY 現金 NT$ --");
    lv_obj_set_pos(lbl_fx_jpy_twd_, 10, 115);
    lv_obj_set_style_text_color(lbl_fx_jpy_twd_, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl_fx_jpy_twd_, &font_puhui_16_4, 0);
...
void PortfolioApplication::CreateWeatherScreen() {
    ...
    const int fc_cell_w = 200;  // fixed width per cell
    const int fc_cell_h = 74;  // fixed height per cell
    ...
    for (int i = 0; i < Weather::FORECAST_DAYS; i++) {
        ...
        // Day label — child of cell_bg, centered horizontally
        det_fc_day_[i] = lv_label_create(cell_bg);
        const char* day_text = weather_.forecast_[i].day[0] ? weather_.forecast_[i].day : "--";
        lv_label_set_text(det_fc_day_[i], day_text);
        lv_obj_set_pos(det_fc_day_[i], (fc_cell_w - 40) / 2, 4);  // center top
        ...
        ESP_LOGI(TAG, "[DEBUG] day[%d]='%s' at cell_y=%d", i, day_text, cell_y);

        // Temp (bottom-left) — child of cell_bg
        det_fc_temp_[i] = lv_label_create(cell_bg);
        snprintf(buf, sizeof(buf), "%d°/%d°", weather_.forecast_[i].high, weather_.forecast_[i].low);
        lv_label_set_text(det_fc_temp_[i], buf);
        lv_obj_set_pos(det_fc_temp_[i], 6, 26);
        ...
        // Rain (bottom-right) — child of cell_bg
        det_fc_rain_[i] = lv_label_create(cell_bg);
        snprintf(buf, sizeof(buf), "🌧️%d%%", weather_.forecast_[i].rain_pct);
        lv_label_set_text(det_fc_rain_[i], buf);
        lv_obj_set_pos(det_fc_rain_[i], fc_cell_w - 62, 26);
    ...
    // Updated time (footer) — create as child of weather_scr_, NOT card2, so it's never clipped
    det_weather_updated_ = lv_label_create(weather_scr_);
    snprintf(buf, sizeof(buf), "更新: %s", weather_.updated[0] ? weather_.updated : "--");
    lv_label_set_text(det_weather_updated_, buf);
    lv_obj_set_pos(det_weather_updated_, 20, card2_y + card2_h + 8);  // below card2
    lv_obj_set_style_text_color(det_weather_updated_, lv_color_hex(0x9E9E9E), 0);
    lv_obj_set_style_text_font(det_weather_updated_, &font_puhui_16_4, 0);
...
void PortfolioApplication::CreatePortfolioScreen() {
    ...
    // ===== y=68: 累計損益 (right-aligned) =====
    lbl = lv_label_create(portfolio_scr_);
    FormatWithCommas(tmp, sizeof(tmp), summary_.total_gain);
    snprintf(buf, sizeof(buf), "%s%s (%+.2f%%)",
        summary_.total_gain >= 0 ? "+" : "-", tmp, summary_.total_gain_pct);
    lv_label_set_text(lbl, buf);
    lv_obj_set_pos(lbl, 460, 75);  // right-align
    lv_obj_set_style_text_color(lbl, summary_.total_gain >= 0 ? lv_color_hex(COLOR_POSITIVE) : lv_color_hex(COLOR_NEGATIVE), 0);
    lv_obj_set_style_text_font(lbl, &lv_font_montserrat_16, 0);

    // ===== y=110: 成本 + 今日 脤跌 (horizontal) =====
    lbl = lv_label_create(portfolio_scr_);
    FormatWithCommas(tmp, sizeof(tmp), summary_.total_cost);
    snprintf(buf, sizeof(buf), "成本 $%s", tmp);
    lv_label_set_text(lbl, buf);
    ...
    det_today_change_ = lv_label_create(portfolio_scr_);
    FormatWithCommas(tmp, sizeof(tmp), fabs(summary_.today_change));
    snprintf(buf, sizeof(buf), "今日 %s%s (%+.2f%%)",
        summary_.today_change >= 0 ? "+" : "-", tmp, summary_.today_change_pct);
    lv_label_set_text(det_today_change_, buf);
    lv_obj_set_pos(det_today_change_, 460, 110);  // right-align
    lv_obj_set_style_text_color(det_today_change_,
        summary_.today_change >= 0 ? lv_color_hex(COLOR_POSITIVE) : lv_color_hex(COLOR_NEGATIVE), 0);
    lv_obj_set_style_text_font(det_today_change_, &font_puhui_16_4, 0);

    // ===== y=140: 分隔線 ===== (用物件代替 line)
    lv_obj_t *divider1 = lv_obj_create(portfolio_scr_);
    lv_obj_set_pos(divider1, 20, 140);
    lv_obj_set_size(divider1, 440, 1);
    lv_obj_set_style_bg_color(divider1, lv_color_hex(COLOR_CARD_LINE), 0);
    lv_obj_set_style_border_width(divider1, 0, 0);
    lv_obj_remove_flag(divider1, LV_OBJ_FLAG_SCROLLABLE);
    ...
    snprintf(buf, sizeof(buf), "$%s  %s%.0f (%+.2f%%)",
        ...
        tw_.gain >= 0 ? "+" : "-", fabs(tw_.gain), tw_.gain_pct);
    lv_label_set_text(det_tw_val_, buf);
    lv_obj_set_pos(det_tw_val_, 20, 175);
    lv_obj_set_style_text_color(det_tw_val_, tw_.gain >= 0 ? lv_color_hex(COLOR_POSITIVE) : lv_color_hex(COLOR_NEGATIVE), 0);
    lv_obj_set_style_text_font(det_tw_val_, &lv_font_montserrat_16, 0);

    // y=200: 台股成本
    lbl = lv_label_create(portfolio_scr_);
    FormatWithCommas(tmp, sizeof(tmp), tw_.cost);
    snprintf(buf, sizeof(buf), "成本 $%s", tmp);
    lv_label_set_text(lbl, buf);
    lv_obj_set_pos(lbl, 20, 205);
    lv_obj_set_style_text_color(lbl, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl, &font_puhui_16_4, 0);

    // ===== y=235: 分隔線 2 =====
    lv_obj_t *divider2 = lv_obj_create(portfolio_scr_);
    lv_obj_set_pos(divider2, 20, 235);
    lv_obj_set_size(divider2, 440, 1);
    lv_obj_set_style_bg_color(divider2, lv_color_hex(COLOR_CARD_LINE), 0);
    lv_obj_set_style_border_width(divider2, 0, 0);
    lv_obj_remove_flag(divider2, LV_OBJ_FLAG_SCROLLABLE);

    // ===== y=250: 💵 美股 =====
    lbl = lv_label_create(portfolio_scr_);
    lv_label_set_text(lbl, "💵 美股");
    lv_obj_set_pos(lbl, 20, 250);
    lv_obj_set_style_text_color(lbl, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl, &font_puhui_16_4, 0);
    ...
    snprintf(buf, sizeof(buf), "$%s  %s%.0f (%+.2f%%)",
        ...
        us_.gain >= 0 ? "+" : "-", fabs(us_.gain), us_.gain_pct);
    lv_label_set_text(det_us_val_, buf);
    lv_obj_set_pos(det_us_val_, 20, 270);
    lv_obj_set_style_text_color(det_us_val_, us_.gain >= 0 ? lv_color_hex(COLOR_POSITIVE) : lv_color_hex(COLOR_NEGATIVE), 0);
    lv_obj_set_style_text_font(det_us_val_, &lv_font_montserrat_16, 0);

    // y=300: 美股成本
    lbl = lv_label_create(portfolio_scr_);
    FormatWithCommas(tmp, sizeof(tmp), us_.cost);
    snprintf(buf, sizeof(buf), "成本 $%s", tmp);
    lv_label_set_text(lbl, buf);
    lv_obj_set_pos(lbl, 20, 300);
    lv_obj_set_style_text_color(lbl, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl, &font_puhui_16_4, 0);

    // ===== y=330: 分隔線 3 =====
    lv_obj_t *divider3 = lv_obj_create(portfolio_scr_);
    lv_obj_set_pos(divider3, 20, 330);
    lv_obj_set_size(divider3, 440, 1);
    lv_obj_set_style_bg_color(divider3, lv_color_hex(COLOR_CARD_LINE), 0);
    lv_obj_set_style_border_width(divider3, 0, 0);
    lv_obj_remove_flag(divider3, LV_OBJ_FLAG_SCROLLABLE);

    // ===== y=345: 📅 年配息 =====
    lbl = lv_label_create(portfolio_scr_);
    lv_label_set_text(lbl, "📅 年配息");
    lv_obj_set_pos(lbl, 20, 345);
    lv_obj_set_style_text_color(lbl, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl, &font_puhui_16_4, 0);

    det_annual_div_ = lv_label_create(portfolio_scr_);
    FormatWithCommas(tmp, sizeof(tmp), summary_.annual_div);
    snprintf(buf, sizeof(buf), "$%s / 年", tmp);
    lv_label_set_text(det_annual_div_, buf);
    lv_obj_set_pos(det_annual_div_, 20, 365);
    lv_obj_set_style_text_color(det_annual_div_, lv_color_hex(COLOR_MONEY), 0);
    lv_obj_set_style_text_font(det_annual_div_, &lv_font_montserrat_22, 0);
    ...
    snprintf(buf, sizeof(buf), "成本 %s  |  現價 %s",
        ...
        summary_.yield_cur[0] ? summary_.yield_cur : "--");
    lv_label_set_text(lbl, buf);
    lv_obj_set_pos(lbl, 20, 400);
    lv_obj_set_style_text_color(lbl, lv_color_hex(COLOR_SUBTEXT), 0);
    lv_obj_set_style_text_font(lbl, &font_puhui_16_4, 0);

    // ===== y=445: 資料時間 (footer) =====
    det_portfolio_updated_ = lv_label_create(portfolio_scr_);
    snprintf(buf, sizeof(buf), "資料: %s", fx_.updated[0] ? fx_.updated : "--");
    lv_label_set_text(det_portfolio_updated_, buf);
    lv_obj_set_pos(det_portfolio_updated_, 20, 445);
    lv_obj_set_style_text_color(det_portfolio_updated_, lv_color_hex(0x9E9E9E), 0);
    lv_obj_set_style_text_font(det_portfolio_updated_, &font_puhui_16_4, 0);
