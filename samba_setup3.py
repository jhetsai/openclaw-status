...
def sudo_run(cmd, timeout=120):
    """Run command via SSH with sudo password sent to stdin."""
    print(f'\n>>> {cmd[:80]}')
    try:
...
[xiaomi_camera]
   ...
   directory mask = 0777
"""
# Write to a temp file first then append
sudo_run('cat > /tmp/samba_share.txt << ' + "'ENDSHAR'")
# Use echo approach instead
...
# smbpasswd with piped input
chan = client.get_transport().open_session()
chan.get_pty()
chan.exec_command('smbpasswd -a backup_user')
time.sleep(0.5)
chan.send(PASS + '\n')  # sudo password
time.sleep(0.3)
